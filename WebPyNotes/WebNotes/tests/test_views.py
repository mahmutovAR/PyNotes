...
from WebPyNotes_settings import NotesPages, NotesMSG, DatabaseAPI
...
class ViewsTest(MongoTestCase):
    ...
    def setUp(self) -> None:
        ...
        self.title_is_taken = NotesMSG.get_msg_title_is_taken()
        ...
        self.invalid_title = 'a' * forms.TITLE_MAX_LENGTH + 'b'
        self.invalid_text = 'a' * forms.TEXT_MAX_LENGTH + 'b'
    ...
    def test_homepage(self):
        ...
        requests.post(DB_TEST_API, json={'test_notes_num': self.test_notes})
    ...
    def test_search_note(self):
        ...
        test_notes = [{'title': '#DtESt_1Atest_4d', 'text': 'n7y2eTest_3cteST_5E'},
                      {'title': '01Test_4dtEst_8h Test_7G', 'text': 'teSt_7G3Jk'},
                      {'title': 'teST_6f gwq', 'text': 'TEst_6f_uteSt_7g'},
                      {'title': '@@wtesT_6F5', 'text': 'asdtest_2bTest_8h'},
                      {'title': 'TeSt_4dtest_8hdtESt_6f', 'text': 'FtEst_7G  1 TeSt_8haqb'},
                      {'title': 'QMtest_6f_rttEst_3ctESt_8h', 'text': 'nTest_5E_=2'}]
        for test_data in test_notes:
            requests.post(DB_API, json=test_data)
        ...
        self.assertContains_search_page(query_one_title_no_text, test_notes[0]['title'])
        ...
        self.assertContains_search_page(query_no_title_one_text, test_notes[3]['title'])
        ...
        self.assertContains_search_page(query_one_title_one_text, test_notes[0]['title'])
        self.assertContains_search_page(query_one_title_one_text, test_notes[5]['title'])
        ...
        self.assertContains_search_page(query_titles_no_text, test_notes[0]['title'])
        self.assertContains_search_page(query_titles_no_text, test_notes[1]['title'])
        self.assertContains_search_page(query_titles_no_text, test_notes[4]['title'])
        ...
        self.assertContains_search_page(query_no_title_texts, test_notes[0]['title'])
        self.assertContains_search_page(query_no_title_texts, test_notes[5]['title'])
        ...
        self.assertContains_search_page(query_titles_one_text, test_notes[2]['title'], 2)
        self.assertContains_search_page(query_titles_one_text, test_notes[3]['title'])
        self.assertContains_search_page(query_titles_one_text, test_notes[4]['title'])
        self.assertContains_search_page(query_titles_one_text, test_notes[5]['title'])
        ...
        self.assertContains_search_page(query_one_title_texts, test_notes[1]['title'], 2)
        self.assertContains_search_page(query_one_title_texts, test_notes[2]['title'])
        self.assertContains_search_page(query_one_title_texts, test_notes[4]['title'])
        ...
        self.assertContains_search_page(query_titles_texts, test_notes[1]['title'])
        self.assertContains_search_page(query_titles_texts, test_notes[3]['title'])
        self.assertContains_search_page(query_titles_texts, test_notes[4]['title'], 2)
        self.assertContains_search_page(query_titles_texts, test_notes[5]['title'])

        for _ in range(6):
            self.assertContains_search_page('test_', test_notes[_]['title'], 2)
    ...
    def assertContains_search_page(self, input_query: str, test_result: str, count_num: int = 1) -> None:
        ...
        The number of matches to check is also specified, default value is ONE."""
    ...
    def test_add_note(self):
        """Tests note creating for valid and invalid entered data.
        The 'note title is taken' case is also tested."""
        note_create = self.client.post(self.add_note_page, {'note_title': self.note_title,
                                                            'note_text': self.note_text})
        api_response = requests.get(DB_API)
        created_note = api_response.json()['notes_all'][0]
        ...
        create_title_is_taken = self.client.post(self.add_note_page, {'note_title': self.note_title})
        self.assertContains(create_title_is_taken, self.title_is_taken)

        note_create_title_error = self.client.post(self.add_note_page, {'note_title': self.invalid_title,
                                                                        'note_text': self.note_text})
        self.assertContains(note_create_title_error, forms.TITLE_MAX_LENGTH_ERROR)
        ...
        self.assertContains(note_create_text_error, forms.TEXT_MAX_VALUE_ERROR)

        note_create_error = self.client.post(self.add_note_page, {'note_title': self.invalid_title,
                                                                  'note_text': self.invalid_text})
        self.assertContains(note_create_error, forms.TITLE_MAX_LENGTH_ERROR)
        self.assertContains(note_create_error, self.invalid_title)
        self.assertContains(note_create_error, forms.TEXT_MAX_VALUE_ERROR)
        self.assertContains(note_create_error, self.invalid_text)

    def test_edit_note(self):
        """Tests note editing for valid and invalid entered data.
        The 'note title is taken' case is also tested."""
        initial_note = requests.post(DB_API, json={'title': self.note_title, 'text': None})
        initial_note_id = initial_note.json()['note_inserted']['id']
        self.initial_note_url = urljoin(DB_API, initial_note_id)

        requests.post(DB_API, json={'title': self.new_note_title, 'text': None})

        self.edit_page_with_id = self.edit_note_page.replace('<data_id>', initial_note_id)

        self.post_and_assert_result(self.note_title, self.note_text)

        self.post_and_assert_result(self.edited_note_title, self.note_text)

        self.post_and_assert_result(self.edited_note_title, self.edited_note_text)
        ...
        try_to_edit_note = self.client.post(self.edit_page_with_id,
                                            {'note_title': self.new_note_title,
                                             'note_text': self.note_text})
        self.assertContains(try_to_edit_note, self.title_is_taken)

        note_edit_title_error = self.client.post(self.edit_page_with_id,
                                                 {'note_title': self.invalid_title,
                                                  'note_text': self.note_text})
        self.assertContains(note_edit_title_error, forms.TITLE_MAX_LENGTH_ERROR)
        ...
        self.assertContains(note_edit_text_error, forms.TEXT_MAX_VALUE_ERROR)
        self.assertContains(note_edit_text_error, self.invalid_text)

        note_edit_error = self.client.post(self.edit_page_with_id,
                                           {'note_title': self.invalid_title,
                                            'note_text': self.invalid_text})
        self.assertContains(note_edit_error, forms.TITLE_MAX_LENGTH_ERROR)
        self.assertContains(note_edit_error, self.invalid_title)
        self.assertContains(note_edit_error, forms.TEXT_MAX_VALUE_ERROR)
        self.assertContains(note_edit_error, self.invalid_text)

    def post_and_assert_result(self, input_title: str, input_text: str) -> None:
        """Additional function to check if the response contains a check values."""
        post_note_edit = self.client.post(self.edit_page_with_id,
                                          {'note_title': input_title, 'note_text': input_text})
        self.assertContains(post_note_edit, self.note_was_edited)

        api_response = requests.get(self.initial_note_url)
        edited_note = api_response.json()['note_content']

        self.assertEqual(edited_note['title'], input_title)
        self.assertEqual(edited_note['text'], input_text)

    def test_delete_note(self):
        """Tests deleting of the note."""
        test_notes = [{'title': 'test note 01', 'text': 'text_ABC'},
                      {'title': 'test note 02', 'text': 'text 123'},
                      {'title': 'test note 03', 'text': 'text ONE-TWO-THREE'},
                      {'title': 'test note 04', 'text': None}]
        for test_data in test_notes:
            requests.post(DB_API, json=test_data)

        note_added = requests.post(DB_API, json={'title': self.note_title, 'text': self.note_text})
        note_id = note_added.json()['note_inserted']['id']

        api_response = requests.get(DB_API)
        total_notes = len(api_response.json()['notes_all'])
    ...
    def test_view_note_content(self):
        ...
        The 'note not found' case is also checked."""
        note_added = requests.post(DB_API, json={'title': self.note_title, 'text': self.note_text})
        note_1_id = note_added.json()['note_inserted']['id']

        test_url = self.note_content_page.replace('<data_id>', note_1_id)
        ...
        note_added = requests.post(DB_API, json={'title': note_with_only_title, 'text': None})
        note_2_id = note_added.json()['note_inserted']['id']

        response = self.client.get(self.note_content_page.replace('<data_id>', note_2_id))
        self.assertContains(response, 'Title:')
        self.assertContains(response, note_with_only_title)
        self.assertNotContains(response, 'Text:')

        self.client.get(self.delete_note_page.replace('<data_id>', note_1_id))
        response = self.client.get(self.note_content_page.replace('<data_id>', note_1_id))
        self.assertContains(response, 'Note not found')

    def test_database_error(self):
        """Tests displaying of the Server Error page."""
